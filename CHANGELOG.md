...
## [1.10.0] — 2026-09-21 (движение ассетов)

### Added
- **Рекурсивный Apply Naming** (Maya + Blender) — пресет применяется к выделению
  и всей ветке детей; идемпотентность пресета защищает от двойных суффиксов,
  объекты вне выделенной ветки не трогаются
- **Apply Naming on Import** (Blender) — галка в панели (включена по умолчанию):
  Import Bridge FBX сразу приводит имена всех новых объектов к активному пресету;
  отключается для чужих ассетов
- Живой транспорт-тест: иерархия локаторов Maya → FBX → Blender → валидация
  STUKACH-иерархией без `missing_grp_suffix`

### Changed
- Blender-манифест 1.9.0 → 1.10.0
